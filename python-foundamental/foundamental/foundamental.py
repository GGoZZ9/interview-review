...
* */
/**
* 深拷贝和浅shallow拷贝之间的区别是什么？
* 深拷贝就是将一个对象拷贝到另一个对象中，这意味着如果你对一个对象的拷贝做出改变时，不会影响impact原对象。在Python中，我们使用函数deepcopy()执行深拷贝，导入模块copy
* copy.deepcopy(a)
* 浅拷贝则是将一个对象的引用reference拷贝到另一个对象上，所以如果我们在拷贝中改动，会影响到原对象
* */
'''
'''
__开始的是私有变量或私有函数
...
列表和元组之间的区别是？
列表是可变的variable，而tuple元组是不可变的 immutable
'''
'''
解释一下Python中的三元运算子 ternary operations
...
解释一下Python中的继承 inherit
单继承：一个类继承自from 单个基类 base class
多继承：一个类继承自多个基类
多级继承：一个类继承自单个基类，后者 latter 则继承自另一个基类
分层继承：多个类继承自单个基类
...
当退出Python时，是否释放全部内存？
答案是No。循环引用 Circular references 其它对象或引用自全局命名空间的对象的模块，在Python退出时并非完全释放。
另外，也不会释放C库保留 retain 的内存部分。
...
当我们不知道向函数传递多少参数时，比如我们想传递一个列表或元组，我们就使用*args。
在我们不知道该传递多少关键字参数时，使用**kwargs来收集关键字参数。
可变参数允许你传入0个或任意个参数，这些可变参数在函数调用时自动组装 assemble 为一个tuple,而关键字参数允许你传入0个或任意个含参数名的参数，这些关键字参数在函数内部自动组装为一个dict。
...
怎么移除一个字符串中的前导空格 Leading spaces ？ '  Ayushi '.lstrip(), 后导空格 Trailing spaces  '  Ayushi '.rstrip()
...
Python有五个标准的数据类型：
Numbers（数字） the first letter is capitalized
int（有符号整型）
