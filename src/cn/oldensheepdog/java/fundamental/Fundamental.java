 ...
 * */
     ...
     * float从左到右，第一位是符号位，2-9位共8位表示正数位，2的（8-1）次方等于128，后面23位是表示小数的，所以最大值是2^128-1。
     * double类型
     * double在java所在占位不固定，表示范围是  2^-1074 至 2^1024-1 。
     * 308位：4.9E-324 至 1.7976931348623157E308。是float数位的将近10倍，主要用来做复杂运算和天文运算。
     * double从左到右，第一位是符号位，2-12是共11位表示正数位，2的（11-1）次方是1024，剩余20位表示小数，所以最大值是2^1024-1。
     * 整数默认都是整型,小数默认都是double型, 所以表示long型常量时要在后面加L(注意大写,小写容易和1混淆),float型后面需要加F
     * https://blog.css8.cn/post/3693483.html
     *
     */
